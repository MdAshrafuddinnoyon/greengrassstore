...
import { Menu, X, User, ChevronDown, ChevronLeft, ChevronRight, ChevronRight as ChevronRightIcon, Leaf, TreeDeciduous, Flower2, Package, Grid3X3, Shrub, Tag, Gift, Sparkles } from "lucide-react";
...
export const Header = () => {
  ...
  const categories = [{
    ...
    image: flowerPot,
    featured: {
      title: t("header.seasonalBlooms"),
      subtitle: t("header.flowerArrangements"),
      href: "/shop?category=flowers"
    },
    subcategories: [{
      name: "Fresh Flowers",
      label: t("sub.freshFlowers"),
      href: "/shop?category=fresh-flowers",
      icon: Flower2
    }, {
      name: "Artificial Flowers",
      label: t("sub.artificialFlowers"),
      href: "/shop?category=artificial-flowers",
      icon: Flower2
    }, {
      name: "Flower Bouquets",
      label: t("sub.flowerBouquets"),
    ...
    image: bluePot,
    featured: {
      title: t("header.designerPots"),
      subtitle: t("header.premiumCollection"),
      href: "/shop?category=pots&sort=price-desc"
    },
    subcategories: [{
      name: "Fiber Pot",
      label: t("sub.fiberPot"),
      href: "/shop?category=fiber-pot",
      icon: Package
    }, {
      name: "Plastic Pot",
      label: t("sub.plasticPot"),
      href: "/shop?category=plastic-pot",
      icon: Package
    }, {
      name: "Ceramic Pot",
      label: t("sub.ceramicPot"),
      href: "/shop?category=ceramic-pot",
      icon: Package
    }, {
      name: "Terracotta Pot",
      label: t("sub.terracottaPot"),
      href: "/shop?category=terracotta-pot",
      icon: Package
    }]
  }, {
    name: "Greenery",
    label: t("nav.greenery"),
    href: "/shop?category=greenery",
    icon: Shrub,
    image: hangingPlants,
    featured: {
      title: t("header.greenWalls"),
      subtitle: t("header.transformSpace"),
      href: "/shop?category=green-wall"
    },
    subcategories: [{
      name: "Green Wall",
      label: t("sub.greenWall"),
      href: "/shop?category=green-wall",
      icon: Grid3X3
    }, {
      name: "Greenery Bunch",
      label: t("sub.greeneryBunch"),
  ...
  const announcements = [
    t("header.announcement1"),
    t("header.announcement2"),
    t("header.announcement3")
  ];
                ...
                >
                  ...
                  >
                    ...
                    {category.subcategories.length > 0 && (
                      <ChevronDown className={cn("w-3.5 h-3.5 transition-transform duration-200", activeMegaMenu === category.name && "rotate-180")} />
                    )}
                  </Link>
                </div>
              ))}
                  ...
                  <div className="grid grid-cols-12 gap-8">
                    ...
                    <div className="col-span-4">
                      <Link to={activeCategory.href} className="text-lg font-semibold text-[#2d5a3d] mb-4 block hover:underline">
                        {t("header.all")} {activeCategory.label}
                      </Link>
                    ...
                    {activeCategory.featured && (
                      <div className="col-span-4">
                        <div className="bg-gradient-to-br from-[#2d5a3d]/5 to-[#2d5a3d]/10 rounded-2xl p-6">
                          <p className="text-xs uppercase tracking-widest text-[#2d5a3d]/70 mb-2">{t("header.featured")}</p>
                      ...
                      <div className="col-span-4">
                        <Link to={activeCategory.href} className="block group">
                          <div className="aspect-[4/3] rounded-2xl overflow-hidden">
                            <img src={activeCategory.image} alt={activeCategory.label} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                          </div>
                          <p className="mt-3 text-sm font-medium text-gray-900 group-hover:text-[#2d5a3d] transition-colors">
                            {t("header.explore")} {activeCategory.label} {t("header.collection")}
                          </p>
                        </Link>
                      </div>
                    )}
              ...
              <div className="p-5">
                    ...
                    <div key={category.name}>
                      ...
                      >
                        {category.subcategories.length > 0 ? (
                          ...
                          <Link 
                            to={category.href} 
                            className={cn("flex items-center gap-2 py-3 px-3 text-sm font-medium hover:bg-gray-50 rounded-lg transition-colors", category.isSale && "text-red-600")} 
                            onClick={() => setIsMobileMenuOpen(false)}
                          >
                            <category.icon className="w-4 h-4" />
                            {category.label}
                        ...
                        {expandedMobileCategory === category.name && category.subcategories.length > 0 && (
                          ...
                          >
                            <div className="pl-9 pr-3 pb-2 space-y-1">
                              <Link 
                                to={category.href} 
                                className="flex items-center gap-2 py-2 text-sm text-[#2d5a3d] font-medium hover:underline" 
                                onClick={() => setIsMobileMenuOpen(false)}
                              >
                                {t("header.all")} {category.label}
                              </Link>
                              {category.subcategories.map(sub => (
                                <Link 
                                  key={sub.name} 
                                  to={sub.href} 
                                  className="flex items-center gap-2 py-2 text-sm text-gray-600 hover:text-gray-900" 
                                  onClick={() => setIsMobileMenuOpen(false)}
                                >
                ...
                <div className="mt-6 pt-6 border-t border-gray-100">
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors"
                  >
                ...
                <div className="mt-4 space-y-2">
                  <Link 
                    to="/account" 
                    className="flex items-center gap-2 py-3 px-3 text-sm font-medium text-gray-600 hover:bg-gray-50 rounded-lg" 
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
